# =========================================================
# TARMAC GROUP PROJECT - BIOMASS ANALYSIS (ONLY {Broom} BR SHEET GROUPED)
# =========================================================
...
br_clean <- br_data %>%
  ...
  mutate(Group = case_when(
    ...
    TRUE ~ "Other"  # fallback if any unexpected entry
  ))
...
# Step 4: ANOVA to compare Biomass across groups
br_anova <- aov(`Biomass carbon (ug/g)` ~ Group, data = br_clean)
summary(br_anova)

# Optional: Post-hoc test (Tukey HSD if ANOVA significant)
# TukeyHSD(br_anova)
...
ggplot(br_clean, aes(x = Group, y = `Biomass carbon (ug/g)`, fill = Group)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Biomass Carbon (µg/g) by Year Group (BR Samples)", 
       x = "Group (Restoration Year / Existing)", 
       y = "Biomass Carbon (µg/g)")

# Step 6: Histogram (optional)
ggplot(br_clean, aes(x = `Biomass carbon (ug/g)`, fill = Group)) +
  geom_histogram(alpha = 0.6, bins = 15, position = "identity") +
  theme_minimal() +
  labs(title = "Distribution of Biomass Carbon (BR Samples)", 
...
# First, calculate upper and lower limits for error bars
...
ggplot(br_summary, aes(x = Group, y = Mean_Biomass, fill = Group)) +
  geom_col(width = 0.6) +  # Bar plot
  geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2) +  # Error bars
  theme_minimal() +
  labs(title = "Mean Biomass Carbon (µg/g) by Restoration Year / Existing Woodland",
